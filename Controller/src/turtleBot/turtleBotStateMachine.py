...
class TurtleBotSM:
    ...
    def _taskRotateRel(self, angle_deg: float) -> bool:
        if angle_deg is not None:
            ...
            target_heading_rad = self.normAngle(
                current_heading + math.radians(angle_deg)
            )
            ...
            print(
                f"Robot {self.name}: Initiated rotate_relative task by {angle_deg:.2f} deg"
            )
            return True
        else:
            print(
                f"Robot {self.name} ERROR: '{TASK_ROTATE_REL}' task requires 'angle' parameter."
            )
            self._current_action = STATE_IDLE
            return False
    ...
    def _taskMoveRel(self, dx: float, dy: float) -> bool:
        ...
        target_x = (
            current_pos[0]
            + dx * math.cos(current_pos[2])
            - dy * math.sin(current_pos[2])
        )
        target_y = (
            current_pos[1]
            + dx * math.sin(current_pos[2])
            + dy * math.cos(current_pos[2])
        )
        ...
        self.start
        print(
            f"Robot {self.name}: Initiated move_relative task to ({target_x:.2f}, {target_y:.2f})"
        )
        return True
    ...
    def executeTask(self, task: dict) -> bool:
        ...
        if task_type == TASK_MOVE_TO:
            ...
            self._explore_max_stagnation_iterations = task_params.get(
    ...
    def startRotating(self, target_heading_rad: float):
        ...
        print(
            f"Robot {self.name}: Starting rotate to {math.degrees(target_heading_rad):.2f} deg (rad: {target_heading_rad:.2f})"
        )
        self._current_action = ACTION_ROTATING
    ...
    def _updateRotatingAction(self) -> bool:
        ...
        if self._current_action != ACTION_ROTATING:
            return False  # Should be in the correct state
    ...
    def _updateExploreAction(self) -> bool:
        ...
        if self._explore_state == EXPLORE_STATE_START:
            ...
            if self._explore_count >= 10:
                print(
                    f"Robot {self.name}: Stagnation count exceeded. Ending exploration."
                )
            ...
            if selected_candidate:
                ...
                move_init_success = self.startMoveTo(
                    target_world_pos[0], target_world_pos[1], internal_call=True
                )
            ...
            if self._current_action == TASK_MOVE_TO:
                ...
                if hasattr(self, "_move_to_success") and self._move_to_success:
                    ...
                    self._explore_state = (
                        EXPLORE_STATE_SCANNING  # Ga naar volgende ronde van explore
                    )
                else:
                    print(
    ...
    def startMoveTo(self, x: float, y: float, internal_call: bool = False) -> bool:
        ...
        if not internal_call and self._current_action != STATE_IDLE:
            print(
                f"Robot {self.name}: Cannot start move_to, already busy with {self._current_action}"
            )
            return False

        if not internal_call and self._current_action == TASK_MOVE_TO:
            print(
                f"Robot {self.name}: Warning: explore is trying to start move_to, but action is already MOVE_TO. Ignoring?"
            )
            return False
        ...
        if not internal_call and self._current_action != STATE_IDLE:
            print(
                f"Robot {self.name}: Cannot start move_to externally, already busy with {self._current_action}"
            )
            return False
        ...
        self._move_to_target_pos = [x, y]
        self._move_to_state = (
            MOVE_TO_STATE_PLANNING  # Zet init state voor move_to state machine
        )
        self._move_to_path = None
        self._move_to_path_index = 0
